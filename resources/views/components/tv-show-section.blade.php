...
{{-- Usage: <x-movie-section title="🔥 Trending" :items="$tvShows" /> --}}
    ...
    @if ($items->isNotEmpty())
            ...
            <div x-data="{
                ...
                <div class="overflow-hidden -mx-2">
                    <div class="flex transition-transform duration-[600ms] ease-[cubic-bezier(0.25,0.1,0.25,1)]"
                        ...
                        @foreach ($items as $i => $tvShow)
                            <div class="w-1/2 sm:w-1/3 md:w-1/4 lg:w-[calc(100%/6)] shrink-0 px-2">
                                <x-tv-show-card :tvShow="$tvShow" :rank="$showRank ? $i + 1 : null" :hideOriginalTitle="true" />
                            </div>
                        @endforeach
                    </div>
                </div>
        ...
        @else
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                @foreach ($items as $tvShow)
                    <x-tv-show-card :tvShow="$tvShow" :hideOriginalTitle="true" />
                @endforeach
            </div>
        @endif
    @else
