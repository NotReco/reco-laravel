                        ...
                        @foreach ($items as $i => $item)
                            <div class="w-1/2 sm:w-1/3 md:w-1/4 lg:w-[calc(100%/6)] shrink-0 px-2">
                                @if ($item instanceof \App\Models\Movie)
                                    <x-movie-card :movie="$item" :rank="$showRank ? $i + 1 : null" :hideOriginalTitle="true" />
                                @else
                                    <x-tv-show-card :tvShow="$item" :rank="$showRank ? $i + 1 : null" :hideOriginalTitle="true" />
                                @endif
                            </div>
                        @endforeach
                    </div>
        ...
        @else
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                @foreach ($items as $item)
                    @if ($item instanceof \App\Models\Movie)
                        <x-movie-card :movie="$item" :hideOriginalTitle="true" />
                    @else
                        <x-tv-show-card :tvShow="$item" :hideOriginalTitle="true" />
                    @endif
                @endforeach
            </div>
        @endif
