<nav x-data="{
                ...
                @auth
                    ...
                    <div class="relative hidden sm:block" x-data="{
                        ...
                        get newNotifications() {
                            return this.notifications.filter(n => n.is_new);
                        },
                        ...
                        @click.outside="open = false; menuOpen = false">
                        ...
                        <button @click="open = !open; if(open) fetchNotifications()" title="Thông báo"
                            ...
                            <span x-show="unreadCount > 0" x-text="unreadCount > 9 ? '9+' : unreadCount"
                                style="display: none;"
                                class="absolute -top-0.5 -right-0.5 min-w-[16px] h-4 bg-sky-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center px-1"></span>
                        </button>

                        {{-- Notification Dropdown Panel --}}
                        <div x-show="open" x-transition.opacity.duration.200ms style="display: none;"
                            class="absolute right-0 mt-3 w-80 sm:w-96 bg-white border border-gray-200 rounded-xl shadow-xl overflow-hidden z-50 flex flex-col max-h-[90vh]">
                            ...
                            {{-- Body --}}
                            <div class="overflow-y-auto flex-1 pb-3 custom-scrollbar">
                                    ...
                                    <div class="animate-pulse" style="min-height: 160px">
                                        ...
                                        <div class="px-2 mx-2 mb-0.5 py-2.5 flex gap-3">
                                            ...
                                            <div class="flex-1 space-y-2.5 pt-2">
                                                ...
                                                <div class="h-2.5 bg-gray-100 rounded w-20 mt-1"></div>
                                            </div>
                                        </div>
                                    </div>
                                </template>
                                ...
                                <template x-if="!loading">
                                    <div>
                                        ...
                                        <template x-if="notifications.length === 0">
                                            <div class="p-8 text-center text-gray-500 text-[15px] font-semibold flex flex-col items-center justify-center"
                                                style="min-height: 160px">
                                                <img src="{{ asset('storage/images/empty-notifications.svg') }}"
                                                    onload="this.style.display=''"
                                                    onerror="this.style.display='none'"
                                                    style="display:none"
                                                    class="w-24 h-24 mb-4 object-contain opacity-50">
                                                Chuông báo đã được làm mới!
                                                <p class="text-[13px] font-normal text-gray-400 mt-1">Bạn không có thông
                                                    báo nào vào lúc này.</p>
                                            </div>
                                        </template>
                                        ...
                                        <template x-if="newNotifications.length > 0">
                                            <div>
                                                <div class="px-4 pt-3 pb-1 flex items-center justify-between">
                                                    <h4 class="font-bold text-gray-900 text-[16px]">Mới</h4>
                                                    <a href="{{ route('notifications.all') }}"
                                                ...
                                                <template x-for="item in newNotifications" :key="item.id">
                                                    <div @click="if(!item.read_at) await markAsRead(item.id); if(item.data.url) window.location.href = item.data.url;"
                                                        class="px-2 mx-2 mb-0.5 py-2.5 rounded-lg cursor-pointer flex gap-3 transition relative group"
                                                        :class="!item.read_at ? 'bg-sky-50/50 hover:bg-sky-50' :
                                                            'bg-transparent hover:bg-gray-50'">
                                                        <div
                                                            class="w-14 h-14 rounded-full bg-sky-100/50 border border-sky-100 text-sky-500 flex items-center justify-center shrink-0 shadow-sm relative overflow-hidden">
                                                            <div
                                                                x-html="item.data.icon ? item.data.icon : `<svg class='w-6 h-6' fill='none' stroke='currentColor' viewBox='0 0 24 24'><path stroke-linecap='round' stroke-linejoin='round' stroke-width='2' d='M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z'/></svg>`">
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div class="flex items-center justify-center shrink-0 w-8 pr-1">
                                                            <button @click.stop
                                                                class="opacity-0 group-hover:opacity-100 w-8 h-8 rounded-full bg-white border border-gray-200 shadow-sm flex items-center justify-center hover:bg-gray-50 transition absolute right-8 z-10">
                                                            ...
                                                            <div x-show="!item.read_at"
                                                                class="w-3 h-3 rounded-full bg-sky-500 absolute right-3">
                                                            </div>
                                                        </div>
                                                    </div>
                                                </template>
                                        ...
                                        <template x-if="olderNotifications.length > 0">
                                            <div :class="newNotifications.length > 0 ? 'mt-1' : ''">
                                                <div class="px-4 pt-3 pb-1 flex items-center justify-between">
                                                ...
                                                <template x-for="item in olderNotifications" :key="item.id">
                                                    <div @click="if(!item.read_at) await markAsRead(item.id); if(item.data.url) window.location.href = item.data.url;"
                                                        class="px-2 mx-2 mb-0.5 py-2.5 rounded-lg cursor-pointer flex gap-3 transition relative group"
                                                        :class="!item.read_at ? 'bg-sky-50/50 hover:bg-sky-50' :
                                                            'bg-transparent hover:bg-gray-50'">
                                                        <div
                                                            class="w-14 h-14 rounded-full bg-gray-100 border border-gray-100 text-gray-500 flex items-center justify-center shrink-0 relative overflow-hidden">
                                                            <div
                                                                x-html="item.data.icon ? item.data.icon : `<svg class='w-6 h-6' fill='none' stroke='currentColor' viewBox='0 0 24 24'><path stroke-linecap='round' stroke-linejoin='round' stroke-width='2' d='M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z'/></svg>`">
                                                            </div>
                                                        </div>
    ...
    <div x-show="mobileOpen" x-transition:enter="transition ease-out duration-200"
        ...
        <div class="px-4 py-4 space-y-1">
            {{-- Mobile search --}}
            <button @click="$dispatch('open-search'); mobileOpen = false"
                class="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-gray-500 hover:text-gray-900 hover:bg-gray-50 transition-colors mb-2">
