<x-app-layout>
    <x-slot:title>Sửa bài viết — Diễn đàn</x-slot:title>
...
<div class="bg-gray-50 min-h-screen pb-12">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <nav class="flex items-center gap-2 text-sm text-gray-500 mb-8">
            ...
            <a href="{{ route('forum.show', $thread) }}" class="hover:text-gray-900 transition-colors truncate max-w-[200px]">{{ $thread->title }}</a>
            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
            <span class="text-gray-700 font-medium">Sửa bài viết</span>
        </nav>

        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 sm:p-8">
            <h1 class="text-2xl font-display font-bold text-gray-900 mb-6">Sửa bài viết</h1>

            <form action="{{ route('forum.updateThread', $thread) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')

                {{-- Category --}}
                <div x-data="{ open: false, selected: '{{ old('forum_category_id', $thread->forum_category_id) }}', selectedName: '' }" x-init="
                    if (selected) {
                ...
                <div>
                    ...
                    <input id="title" type="text" name="title" value="{{ old('title', $thread->title) }}"
                ...
                <div class="flex items-center justify-between pt-2">
                    <a href="{{ route('forum.show', $thread) }}" class="text-sm text-gray-500 hover:text-gray-900 transition-colors font-medium">
                        ← Quay lại
                    </a>
                    <button type="submit"
