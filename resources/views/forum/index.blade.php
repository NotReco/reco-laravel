<x-app-layout>
    ...
    <div class="bg-gray-50 min-h-screen pb-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8" x-data="forumIndex()" x-init="init()">
                    ...
                    <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
                        ...
                        <div class="p-2 space-y-1">
                            ...
                            <button @click="filterCategory('')"
                                ...
                                :class="activeCategory === '' ? 'bg-sky-50 text-sky-600' :
                                    'text-gray-600 hover:text-gray-900 hover:bg-gray-50'">
                                Tất cả
                                <span class="text-xs px-2 py-0.5 rounded-full font-bold"
                                    :class="activeCategory === '' ? 'bg-sky-100 text-sky-600' :
                            ...
                            @foreach ($categories as $cat)
                                <button @click="filterCategory('{{ $cat->slug }}')"
                                    class="w-full flex items-center justify-between px-4 py-2.5 rounded-xl text-sm font-medium transition-all"
                                    :class="activeCategory === '{{ $cat->slug }}' ? 'bg-sky-50 text-sky-600' :
                                        'text-gray-600 hover:text-gray-900 hover:bg-gray-50'">
                                    {{ $cat->name }}
                                    <span class="text-xs px-2 py-0.5 rounded-full font-bold"
                                        :class="activeCategory === '{{ $cat->slug }}' ? 'bg-sky-100 text-sky-600' :
                                            'bg-gray-100 text-gray-500'">
                                        {{ $cat->threads_count }}
                                    </span>
                                </button>
                        ...
                        <div>
                            ...
                            <template x-for="thread in threads" :key="thread.id">
                                <a :href="thread.url"
                                    ...
                                    <div class="flex items-start gap-4">
                                        <div class="relative group w-10 h-10 shrink-0 shadow-sm transition-all duration-300"
                                            :class="[!thread.user.active_frame ? 'ring-2 ring-white rounded-full' : '']">
                                            <div
                                                class="w-full h-full rounded-full bg-gradient-to-br from-sky-400 to-sky-600 overflow-hidden flex items-center justify-center transition-all duration-300"
                                                :class="[thread.user.active_frame ? 'scale-[1.0475]' : '']">
                                                ...
                                                <template x-if="!thread.user.avatar">
                                                    <span class="text-xs font-bold text-white"
                                                        x-text="thread.user.initial"></span>
                                                </template>
                                            </div>
                                        ...
                                        <div class="flex-1 min-w-0">
                                            ...
                                            <div class="flex items-center gap-2 flex-wrap">
                                                <template x-if="thread.is_pinned">
                                                    <span
                                                        class="inline-flex items-center gap-1 text-xs font-semibold text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full">
                                                        📌 Ghim
                                                    </span>
                                                </template>
                                            ...
                                            <div class="flex items-center gap-3 mt-1.5 text-xs text-gray-500">
                                                <span
                                                    ...
                                                    x-text="thread.category.name"></span>
                                                <span class="font-medium text-gray-700"
                                                    x-text="thread.user.name"></span>
                                                <template x-if="thread.user.active_title">
                                                    <span
                                                        class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10px] font-bold border"
                                                ...
                                                <span class="flex items-center gap-1 text-rose-500">
                                                    <svg class="w-3.5 h-3.5 fill-current" stroke="currentColor"
                                                        viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                                            stroke-width="1.5"
                                        ...
                                        <template x-if="thread.latest_reply">
                                            <div class="hidden sm:block text-right shrink-0">
                                                <p class="text-xs text-gray-600 truncate max-w-[120px] font-medium"
                                                    x-text="thread.latest_reply.user_name"></p>
                                                <p class="text-xs text-gray-400"
                                                    x-text="thread.latest_reply.created_at"></p>
                                            </div>
                                        </template>
                                    </div>
                                </a>
