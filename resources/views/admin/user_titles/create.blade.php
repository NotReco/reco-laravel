<x-admin-layout pageTitle="Thêm Danh Hiệu">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h2 class="text-xl font-semibold text-white">Thêm Danh Hiệu Mới</h2>
            <p class="text-sm text-dark-400 mt-1">Tạo danh hiệu mới để cấp cho người dùng</p>
        </div>
    ...
    <div class="bg-dark-900 border border-dark-800 rounded-2xl shadow-sm p-6 sm:p-8">
        <form action="{{ route('admin.user-titles.store') }}" method="POST" class="max-w-2xl space-y-6">
            @csrf
            ...
            <div class="grid sm:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex gap-2">
                        <input type="color" id="color_picker" class="h-11 w-11 rounded cursor-pointer border-0 p-0" oninput="document.getElementById('color_hex').value = this.value" value="{{ old('color_hex', '#38bdf8') }}">
                        <input type="text" id="color_hex" name="color_hex" 
                               class="w-full bg-dark-950 border border-dark-800 rounded-xl text-white placeholder-dark-600 focus:ring-sky-500 focus:border-sky-500 px-4 py-2.5 transition-colors" 
                               value="{{ old('color_hex', '#38bdf8') }}" required oninput="document.getElementById('color_picker').value = this.value">
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-2">
                ...
                <textarea id="description" name="description" rows="3" 
                          class="w-full bg-dark-950 border border-dark-800 rounded-xl text-white placeholder-dark-600 focus:ring-sky-500 focus:border-sky-500 px-4 py-2.5 transition-colors resize-none">{{ old('description') }}</textarea>
            </div>
