<x-admin-layout title="Danh hiệu" pageTitle="Quản lý danh hiệu">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            ...
            <p class="text-sm text-dark-400 mt-1">Tổng: {{ $titles->total() }} danh hiệu</p>
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        @forelse ($titles as $title)
            <div
                class="bg-dark-900 border border-dark-800 rounded-2xl p-5 flex flex-col gap-4 hover:border-dark-600 transition-colors group">

                {{-- Preview badge --}}
                <div class="flex items-center gap-3">
                    <div
                        class="w-10 h-10 rounded-full bg-gradient-to-br from-slate-600 to-slate-800 flex items-center justify-center overflow-hidden text-sm font-bold text-white shrink-0">
                        @if (Auth::user()->avatar)
                    ...
                    <div class="flex flex-col gap-1 min-w-0">
                        ...
                        <span class="text-xs font-bold px-2 py-0.5 rounded-full w-fit max-w-full break-words whitespace-normal"
                ...
                <div class="flex-1">
                    <div class="flex items-center gap-2 mb-1">
                        <div class="w-4 h-4 rounded-full shrink-0" style="background-color: {{ $title->color_hex }}">
                        </div>
                ...
                <div class="flex items-center justify-between pt-3 border-t border-dark-800">
                    <div class="flex items-center gap-2">
                        @if ($title->is_active)
                            <span
                                class="inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
                                <span class="w-1.5 h-1.5 rounded-full bg-emerald-400 inline-block"></span>
                                Hiển thị
                            </span>
                        @else
                            <span
                                class="inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full bg-red-500/10 text-red-400 border border-red-500/20">
                    ...
                    <div class="flex items-center gap-1">
                        <a href="{{ route('admin.user-titles.edit', $title) }}"
                            class="p-1.5 text-dark-500 hover:text-sky-400 hover:bg-sky-400/10 rounded-lg transition-colors"
                            title="Sửa">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <form action="{{ route('admin.user-titles.destroy', $title) }}" method="POST"
                            class="inline-block" id="del-title-{{ $title->id }}">
                            @csrf @method('DELETE')
                            <button type="button"
                                class="p-1.5 text-dark-500 hover:text-red-400 hover:bg-red-400/10 rounded-lg transition-colors"
                                title="Xóa"
                                @click="$dispatch('admin-confirm', { title: 'Xóa danh hiệu', message: 'Xóa «{{ addslashes($title->name) }}»? Hành động này không thể hoàn tác.', formId: 'del-title-{{ $title->id }}' })">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
