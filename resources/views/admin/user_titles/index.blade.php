<x-admin-layout title="Danh hiệu" pageTitle="Quản lý danh hiệu">
    <div class="flex justify-between items-center mb-6">
        <div>
            ...
            <p class="text-sm text-dark-400 mt-1">Quản lý các danh hiệu để cấp cho người dùng</p>
        </div>
        <a href="{{ route('admin.user-titles.create') }}" class="inline-flex items-center justify-center gap-2 px-4 py-2 bg-sky-500 text-white text-sm font-semibold rounded-xl hover:bg-sky-600 transition-all shadow-sm">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm whitespace-nowrap">
                <thead class="bg-dark-800/50 text-dark-300">
                    <tr>
                        <th class="px-6 py-4 font-medium tracking-wider">Màu sắc</th>
                        <th class="px-6 py-4 font-medium tracking-wider">Tên danh hiệu</th>
                ...
                <tbody class="divide-y divide-dark-800">
                    @forelse ($titles as $title)
                        <tr class="hover:bg-dark-800/30 transition-colors">
                            <td class="px-6 py-4">
                                <div class="w-8 h-8 rounded-full border border-dark-700" style="background-color: {{ $title->color_hex }}"></div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex flex-col">
                                    <span class="font-semibold text-white">{{ $title->name }}</span>
                                    <span class="text-xs text-dark-400">{{ Str::limit($title->description, 50) }}</span>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <form action="{{ route('admin.user-titles.destroy', $title) }}" method="POST" class="inline-block"
                                          onsubmit="return confirm('Bạn có chắc muốn xóa danh hiệu này?')">
                                        @csrf @method('DELETE')
                                        <button type="submit" 
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center">
                                <h3 class="text-sm font-medium text-white mb-1">Chưa có danh hiệu</h3>
                                <p class="text-sm text-dark-400">Hãy thêm danh hiệu đầu tiên.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
