...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-8">
    @php
        $cards = [
            ['label' => 'Tổng phim',        'value' => $stats['movies'],          'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 4v16M17 4v16M3 8h4m10 0h4M3 12h18M3 16h4m10 0h4M4 20h16a1 1 0 001-1V5a1 1 0 00-1-1H4a1 1 0 00-1 1v14a1 1 0 001 1z"/>', 'color' => 'blue',    'href' => null],
            ['label' => 'Tổng reviews',     'value' => $stats['reviews'],         'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/>', 'color' => 'amber',  'href' => null],
            ['label' => 'Tổng users',       'value' => $stats['users'],           'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/>',      'color' => 'sky',    'href' => null],
            ['label' => 'Reviews hôm nay',  'value' => $stats['today_reviews'],   'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"/>',                                                                                                           'color' => 'emerald','href' => null],
            ['label' => 'Chờ duyệt',        'value' => $stats['pending_reviews'], 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>',                                                                                               'color' => $stats['pending_reviews'] > 0 ? 'red' : 'slate', 'href' => route('admin.reviews.index', ['status' => 'pending'])],
            ['label' => 'Diễn đàn',         'value' => $stats['forum_threads'],   'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8h2a2 2 0 012 2v6a2 2 0 01-2 2h-2v4l-4-4H9a2 2 0 01-2-2V10a2 2 0 012-2"/>',                                                               'color' => 'violet', 'href' => null],
        ];
    @endphp

    @foreach($cards as $card)
        @php $wrap = $card['href'] ? 'a' : 'div'; @endphp
        <{{ $wrap }} {{ $card['href'] ? 'href="'.$card['href'].'"' : '' }} class="card p-5 {{ $card['href'] ? 'hover:border-'.$card['color'].'-500/40 transition-colors cursor-pointer' : '' }}">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    @if($card['href'] && $card['value'] > 0)
                        <p class="text-xs text-{{ $card['color'] }}-400 mt-1">Nhấn để xem →</p>
                    @endif
                </div>
