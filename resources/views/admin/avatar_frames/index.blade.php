<x-superadmin-layout title="Khung Avatar" pageTitle="Quản lý khung Avatar">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            ...
            <p class="text-sm text-dark-400 mt-1">Tổng: {{ $frames->total() }} khung</p>
        </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5">
        @forelse ($frames as $frame)
            <div
                class="bg-dark-900 border border-dark-800 rounded-2xl overflow-hidden hover:border-dark-600 transition-all group flex flex-col">

                {{-- Frame preview --}}
                <div
                    class="relative flex items-center justify-center bg-gradient-to-br from-dark-800 to-dark-950 py-6 px-4">
                    {{-- Mock avatar underneath --}}
                    <div class="relative w-20 h-20 shrink-0">
                        {{-- Avatar bg --}}
                        <div
                            class="w-full h-full rounded-full bg-gradient-to-br from-sky-500 to-blue-700 flex items-center justify-center overflow-hidden text-3xl font-bold text-white scale-[1.0475]">
                            @if (Auth::user()->avatar)
                    ...
                    <div class="absolute top-2.5 right-2.5">
                        @if ($frame->is_active)
                            <span class="w-2.5 h-2.5 rounded-full bg-emerald-400 block ring-2 ring-dark-900"
                                title="Hoạt động"></span>
                        @else
                            <span class="w-2.5 h-2.5 rounded-full bg-red-400 block ring-2 ring-dark-900"
                                title="Ẩn"></span>
                        @endif
                    </div>
                </div>
                    ...
                    <div class="flex items-center justify-center gap-1 mt-auto pt-1 border-t border-dark-800">
                        ...
                        <form action="{{ route('admin.avatar-frames.destroy', $frame) }}" method="POST"
                            ...
                            <button type="button"
                                ...
                                @click="$dispatch('admin-confirm', { title: 'Xóa khung avatar', message: 'Xóa khung «{{ addslashes($frame->name) }}»? Hành động này không thể hoàn tác.', formId: 'del-frame-{{ $frame->id }}' })">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
