<x-admin-layout pageTitle="Sửa Khung Avatar">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h2 class="text-xl font-semibold text-white">Chỉnh Sửa Khung</h2>
        </div>
    ...
    <div class="bg-dark-900 border border-dark-800 rounded-2xl shadow-sm p-6 sm:p-8">
        <form action="{{ route('admin.avatar-frames.update', $frame) }}" method="POST" enctype="multipart/form-data" class="max-w-2xl space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-4">
                <label for="image" class="block text-sm font-medium text-dark-300">Đổi File (Tùy chọn)</label>
            ...
            <div class="flex items-center h-full">
                <label class="flex items-center gap-3 cursor-pointer group">
                    <div class="relative flex items-center justify-center">
                        <input type="checkbox" name="is_active" value="1" class="peer sr-only" {{ old('is_active', $frame->is_active) ? 'checked' : '' }}>
