            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex flex-col md:flex-row gap-8 -mt-52 mb-10">
                    ...
                    <div class="flex-1 pt-4 md:pt-6 relative z-10">
                        ...
                        {{-- Rating + Actions --}}
                        @php
                            $topMoods = \App\Models\MovieVibe::where('movie_id', $movie->id)
                                ->whereNotNull('mood')
                                ->groupBy('mood')
                                ...
                                ->toArray();
                            $myVibe = auth()->check()
                                ? \App\Models\MovieVibe::where('movie_id', $movie->id)
                        ...
                        <div class="flex flex-col gap-4" x-data="{
                            ...
                            <div class="flex flex-wrap items-center gap-4">
                                ...
                                <button @click="toggleFavorite()" title="Yêu thích"
                                    ...
                                    <svg class="w-5 h-5" :fill="isFavorited ? 'currentColor' : 'none'"
                                        fill="{{ auth()->check() && $movie->favoritedBy->contains(auth()->id()) ? 'currentColor' : 'none' }}"
                                        stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <div x-data="{ showSharePopup: false }">
                                    ...
                                    <template x-teleport="body">
                                        <div x-show="showSharePopup" 
                                            ...
                                            <p class="text-sm font-medium text-white">Đã sao chép liên kết phim!</p>
                                ...
                                @php
                                    $wlStatus = auth()->check()
                                        ? $movie->watchlistedBy->where('id', auth()->id())->first()?->pivot?->status ??
                                            ''
                                        : '';
                            ...
                            {{-- Include Modal --}}
                            <x-movie-vibe-modal :movie="$movie" :myMood="$myMood" :myTone="$myTone" />
                        ...
                        @if ($movie->reviews->isNotEmpty())
                            <section>
                                <div class="flex items-center justify-between mb-4">
                                    ...
                                    <span
                                        class="text-xs font-semibold text-gray-500 bg-gray-100 px-2.5 py-1 rounded-full">{{ count($movie->reviews) }}
                                        đánh giá</span>
                                </div>
                ...
                @if ($relatedMovies->isNotEmpty())
                    <div class="border-t border-gray-100 pt-10 pb-4">
                        <x-movie-section title="Phim liên quan" :items="$relatedMovies" layout="grid" />
                    </div>
                @endif
